...
const ROLE_META: Record<UserRole, { label: string; color: string; bg: string; border: string }> = {
  ...
  operator:   { label: 'Operator',   color: 'var(--acc)', bg: 'var(--acc-10)',   border: 'var(--acc-30)' },
...
function Avatar({ name, size = 38 }: { name: string; size?: number }) {
  ...
  return (
    <div style={{
      ...
      fontSize: size * 0.32, fontWeight: 700, color: 'var(--acc)',
  ...
  return (
    <div style={{
      ...
      <div>
        ...
        <div style={{ fontSize: 11, color: 'var(--txt-3)', marginTop: 3, fontFamily: 'DM Sans, sans-serif' }}>{label}</div>
      </div>
    </div>
  );
}
    ...
    <AppLayout>
      <div style={{ flex: 1, overflow: 'auto', padding: '32px 40px', background: 'var(--srf-0)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <button
              ...
              style={{
                ...
                borderRadius: 8, color: 'var(--txt-2)', cursor: 'pointer', fontSize: 13,
                fontFamily: 'DM Sans, sans-serif',
              }}
            >
            ...
            <div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 24, color: 'var(--txt-1)' }}>
                {t('users.title')}
              </div>
              <div style={{ fontSize: 13, color: 'var(--txt-3)', marginTop: 3, fontFamily: 'DM Sans, sans-serif' }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            {canCreate && (
              <button onClick={() => setModalUser(null)} style={{
                ...
                borderRadius: 8, border: 'none', background: 'var(--acc)', color: 'var(--srf-0)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 28 }}>
          <StatCard icon={<Users size={18} />}    label={t('users.total_users')}  value={counts.total}      color="var(--acc)" />
          ...
          <StatCard icon={<Users size={18} />}    label={t('users.operators')}    value={counts.operator}   color="var(--acc)" />
        ...
        {/* ── Table card ── */}
        <div style={{
          background: 'var(--srf-1)', border: '1px solid var(--acc-10)',
          borderRadius: 14, overflow: 'hidden',
        }}>
          {/* Controls bar */}
          <div style={{
            display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px',
            borderBottom: '1px solid var(--bdr-06)', flexWrap: 'wrap',
          }}>
            {/* Search */}
            <div style={{ position: 'relative', flex: '1 1 220px', minWidth: 180 }}>
              <Search size={13} style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', color: 'var(--txt-3)' }} />
              <input
                value={search}
                ...
                style={{
                  ...
                  borderRadius: 7, color: 'var(--txt-1)', fontSize: 13, outline: 'none',
                  fontFamily: 'DM Sans, sans-serif',
                }}
              />
            </div>
              ...
              {(['all', 'superadmin', 'admin', 'operator', 'viewer'] as const).map(r => {
                ...
                return (
                  <button key={r} onClick={() => setRoleFilter(r)} style={{
                    ...
                    background: active ? (meta?.bg || 'var(--acc-15)') : 'var(--fill-04)',
                    color: active ? (meta?.color || 'var(--acc)') : 'var(--txt-3)',
                    transition: 'all .15s',
                  }}>
          ...
          <div style={{
            ...
            borderBottom: '1px solid var(--fill-05)',
          ...
          {/* Loading */}
          {isLoading && (
            <div style={{ padding: '48px 20px', textAlign: 'center', color: 'var(--txt-3)', fontSize: 14, fontFamily: 'DM Sans, sans-serif' }}>
              {t('users.loading')}
            </div>
          )}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderBottom: idx < filtered.length - 1 ? '1px solid var(--fill-04)' : 'none',
                  background: isMe ? 'var(--acc-03)' : 'transparent',
                  alignItems: 'center',
                  transition: 'background .15s',
                }}
                onMouseEnter={e => { if (!isMe) (e.currentTarget as HTMLDivElement).style.background = 'var(--fill-02)'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.background = isMe ? 'var(--acc-03)' : 'transparent'; }}
              >
                {/* User info */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--txt-1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'DM Sans, sans-serif' }}>
                        {u.full_name}
                      </span>
                      {isMe && (
                        <span style={{ fontSize: 9, padding: '1px 6px', borderRadius: 3, background: 'var(--acc-15)', color: 'var(--acc)', fontWeight: 700, letterSpacing: 0.5, flexShrink: 0 }}>
                          {t('users.you')}
                        </span>
                      )}
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--txt-3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'DM Sans, sans-serif' }}>
                      {u.email}
                    </div>
                    {u.phone && (
                      <div style={{ fontSize: 11, color: 'var(--txt-4)', fontFamily: 'JetBrains Mono, monospace', marginTop: 1 }}>{u.phone}</div>
                    )}
                ...
                {/* Last login */}
                <div style={{ fontSize: 12, color: 'var(--txt-3)', fontFamily: 'JetBrains Mono, monospace' }}>
                  {u.last_login ? timeAgo(u.last_login) : <span style={{ color: 'var(--txt-4)' }}>{t('users.never')}</span>}
                </div>

                {/* Created */}
                <div style={{ fontSize: 12, color: 'var(--txt-3)', fontFamily: 'DM Sans, sans-serif' }}>
                ...
                <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                  {canEdit && (
                    <button onClick={() => { setActionError(''); setModalUser(u); }} title={t('users.edit')} style={{
                      ...
                      borderRadius: 6, border: '1px solid var(--bdr-10)',
                  ...
                  {canToggle && (
                    <button onClick={() => toggleActive(u.id, !u.is_active)} title={u.is_active ? 'Deactivate' : 'Activate'} style={{
                      ...
                      borderRadius: 6, border: '1px solid var(--bdr-08)',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', marginBottom: 20 }}>
              ...
              <div>
                <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 16, color: 'var(--txt-1)', marginBottom: 6 }}>
                  {t('users.delete_title')}
                </div>
                <div style={{ fontSize: 13, color: 'var(--txt-2)', fontFamily: 'DM Sans, sans-serif', lineHeight: 1.6 }}>
                  <strong style={{ color: 'var(--txt-1)' }}>{confirmDelete.full_name}</strong> ({confirmDelete.email}) {t('users.delete_body')}
                </div>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
